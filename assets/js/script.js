...
counters.forEach(counter => {
  ...
  const circle = document.querySelector(".timeline__circle");
  const container = document.querySelector(".timeline__container");

  function updateTimelineCircle() {
    ...
    items.forEach((item, index) => {
      ...
      // 🔵 ACTIVE ITEM
      if (itemTop <= viewportMiddle && itemBottom >= viewportMiddle) {
        const progress =
          (viewportMiddle - itemTop) / itemHeight;

        circleTop =
          item.offsetTop + progress * itemHeight;
      }

      // 🟢 COMPLETED ITEM (circle stops at bottom)
      if (itemBottom < viewportMiddle) {
        circleTop =
  ...
  window.addEventListener("scroll", updateTimelineCircle);
  window.addEventListener("load", updateTimelineCircle);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const durations = document.querySelectorAll(".pricing-card__duration");
  ...
  toggleButtons.forEach((btn, index) => {
    btn.addEventListener("click", function () {
      ...
      prices.forEach((price, i) => {
        ...
        setTimeout(() => {
          price.textContent = pricingData[type][i];
          price.style.opacity = "1";
        }, 200);
      });

      // Update duration text (skip Free)
      durations.forEach(duration => {
        if (type === "monthly") {
          duration.textContent = "/month, per user";
        } else {
          duration.textContent = "/year, per user";
        }
      });
...
const counter = document.getElementById("joinedCount");
